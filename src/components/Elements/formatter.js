...
class EditorFormatter extends React.Component {
  ...
  renderFormatter = () => {
    ...
    switch (columnType) {
      case CellType.TEXT: {
        let textFormatter = (
          <TextFormatter value={row[columnKey]} containerClassName="orgchart-text-editor" />
        );
        if (!row[columnKey]) {
      ...
      case CellType.COLLABORATOR: {
        let collaboratorFormatter = (
          <CollaboratorFormatter
            ...
            containerClassName="orgchart-text-editor"
          />
        );
      ...
      case CellType.GEOLOCATION: {
        let geolocationFormatter = (
          <GeolocationFormatter value={row[columnKey]} containerClassName="orgchart-text-editor" />
        );
        if (!row[columnKey]) {
      ...
      case CellType.LINK_FORMULA: {
        ...
        let formulaFormatter = (
          <FormulaFormatter
            ...
            column={column}
            collaborators={collaborators}
            containerClassName="orgchart-formula-container"
          />
        );
        if (!formulaValue) {
      ...
      case CellType.LINK: {
        ...
        let linkFormatter = (
          <LinkFormatter
            ...
            containerClassName="orgchart-link-container"
          />
        );
        if (displayColumnName) {
      ...
      case CellType.AUTO_NUMBER: {
        let autoNumberFormatter = (
          <AutoNumberFormatter value={row[columnKey]} containerClassName="orgchart-text-editor" />
        );
        if (!row[columnKey]) {
      ...
      case CellType.URL: {
        let urlFormatter = (
          <UrlFormatter value={row[columnKey]} containerClassName="orgchart-text-editor" />
        );
        if (!row[columnKey]) {
      ...
      case CellType.EMAIL: {
        let emailFormatter = (
          <EmailFormatter value={row[columnKey]} containerClassName="orgchart-text-editor" />
        );
        if (!row[columnKey]) {
